...
import { groupByDay, filterFalseItem, converToDailyDate } from '../../../helpers/managmentDataHelper';
					...
					<List
						...
						renderItem={(item) => (
							<div style={{ padding: 15 }}>
								{isSortedByDate && typeof item == 'string' && (
									<TitleContainer>{converToDailyDate(item)}</TitleContainer>
								)}
								{typeof item != 'string' && (
									<ListItem item={item} hasImage={hasImage} isSortedByDate={isSortedByDate} />
								)}
							</div>
						)}
					/>
